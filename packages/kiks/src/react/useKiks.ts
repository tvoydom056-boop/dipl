 ...
 */
export function useKiks<TState, TAction extends Action>(store: Store<TState, TAction>): TState;
export function useKiks<TState, TAction extends Action, TResult>(
  store: Store<TState, TAction>,
  selector: Selector<TState, TResult>,
): TResult;
export function useKiks<TState, TResult>(selector: Selector<TState, TResult>): TResult;
export function useKiks<TState, TAction extends Action, TResult = TState>(
  ...
  const resolvedStore = (
    isSelector(storeOrSelector) ? contextStore : (storeOrSelector ?? contextStore)
  ) as Store<TState, TAction> | null;

  if (!resolvedStore) {
    throw new Error("useKiks требует store в аргументах или Provider в дереве компонентов.");
  }
